...
import { ChefHat, Home, ClipboardList, Settings, Menu, X, Star, PenBoxIcon, UploadCloud,Handshake } from 'lucide-react';
...
export function Sidebar({ isOpen, toggleSidebar }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { icon: PenBoxIcon, label: 'CustomizeOrder', path: '/chef-dashboard/customizeorder' },
    { icon: ClipboardList, label: 'Orders', path: '/chef-dashboard/orders' },
    { icon: UploadCloud, label: 'MenuUpload', path: '/chef-dashboard/menuupload' },
    { icon: Handshake, label: 'Delivery Partners', path: '/chef-dashboard/deliverypartners' },
    { icon: Star, label: 'Premium', path: '/chef-dashboard/premium' },
    { icon: Settings, label: 'Settings', path: '/chef-dashboard/settings' },
  ];
      ...
      >
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center space-x-3">
            ...
            <span className={`font-bold text-xl ${!isOpen && 'hidden'}`}>CloudChef</span>
          </div>
          <button
            onClick={toggleSidebar}
